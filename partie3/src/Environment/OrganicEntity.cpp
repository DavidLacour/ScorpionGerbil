...
OrganicEntity::OrganicEntity( const Vec2d& position, const double& taille , const double& energie) : CircularCollider(position,
    ...
    age_limit_(sf::seconds(10000)), energie_depense_base_(0.1) {}



OrganicEntity::OrganicEntity( const OrganicEntity& OE ) : OrganicEntity( OE.getPosition(),OE.energie_,OE.energie_) {}

OrganicEntity::OrganicEntity( const Vec2d& position, const double& taille , const double& energie, const sf::Time& ageLimit)
    : CircularCollider(position, taille), energie_(energie),  age_(sf::Time::Zero), age_limit_(ageLimit)
{
