...
{
    Animal::update( dt);
    if(energie_>=(getRadius()/getAppConfig().scorpion_size*5*getAppConfig().scorpion_energy_initial)) {
        setEnergy(getEnergie()/2);
        getAppEnv().addEntity(new Scorpion(getPosition()-getDirection()*getRadius()*1.2, this->getEnergie(),true ));
    }
...
bool Scorpion::eatableBy(Scorpion const* scorp ) const
{
...
double Scorpion::normalPositif(double valeur, double variance)
{
...
double  Scorpion::normalPositif(double valeur)
{
    return normalPositif(valeur,valeur*valeur/3);
}
